...
export default function DauGraphScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('오류', e instanceof Error ? e.message : 'DAU 그래프 데이터를 불러오지 못했습니다.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const points = useMemo(
    () =>
      [...rows]
        .reverse()
        .map((r) => ({ date: r.count_date.slice(5), value: Number(r.dau || 0) })),
    [rows],
  );

  const latest = rows[0]?.dau ?? 0;
  const prev = rows[1]?.dau ?? 0;
  const delta = latest - prev;

  return (
    <ThemedView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <ThemedText type="title">DAU 그래프</ThemedText>
        <ThemedText style={styles.sub}>날짜별 활성사용자 수 추이</ThemedText>
        ...
        <MetricTrendChart points={points} color="#2563eb" />
      </ScrollView>
    </ThemedView>
  );
}
