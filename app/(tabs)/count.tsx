...
export default function ParkingCountScreen() {
  ...
  return (
    <ThemedView style={styles.screen}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <ThemedText style={styles.sub}>날짜를 누르면 상세 지표를 확인할 수 있습니다.</ThemedText>
        ...
        <View style={styles.list}>
          {rows.map((row) => (
            <Pressable
              key={row.count_date}
              onPress={() => router.push({ pathname: '/countdetail', params: { countDate: row.count_date } } as any)}
              style={({ pressed }) => [styles.item, { opacity: pressed ? 0.9 : 1 }]}>
              <ThemedText style={styles.date}>{row.count_date}</ThemedText>
              <ThemedText>DAU: {row.dau}</ThemedText>
